...
export function PluginAssets({ id, revision }: { id: string; revision: string }) {
  ...
  return <details className="mt-3 text-xs"><summary className="cursor-pointer">声明资源（{assets.length}）</summary>
    <p className="mt-2 text-text-muted">可导出审查。目标业务的自动安装尚未接入，导出不会覆盖应用资源。</p>
    {assets.map(asset => <div key={`${asset.source}:${asset.targetKind}:${asset.target}`} className="mt-2 flex gap-2 rounded bg-bg p-2">
      <div className="min-w-0 flex-1 break-all"><b>{asset.source}</b><p>{asset.targetKind} / {asset.target} · {asset.byteLength} B</p><p className="font-mono text-text-muted">{asset.sha256}</p></div>
