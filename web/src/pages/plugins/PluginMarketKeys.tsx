...
export function PluginMarketKeys({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  return <details className="mb-4 rounded-xl border border-border bg-card p-4 text-sm">
    <summary className="cursor-pointer font-medium">市场可信公钥</summary>
    <p className="mt-2 text-xs text-text-muted">填写发行者提供的 Ed25519 公钥，并核对指纹。只需公钥，不要填写私钥。来源配置中的密钥 ID 必须与这里一致。</p>
    ...
    <div className="mt-3 space-y-2">{keys.map(key => <div key={key.id} className="rounded bg-bg p-3 text-xs">
      <div className="flex items-center gap-2"><b>{key.id}</b><span>{key.enabled ? '已信任' : '已撤销'}</span>
        {key.enabled && <button disabled={busy} onClick={() => setConfirm(key.id)} className="ml-auto text-danger">撤销信任</button>}
      </div>
      ...
      {confirm === key.id && <div className="mt-2 flex flex-wrap gap-3"><span>撤销后将清除使用此密钥的市场缓存索引。</span>
        <button disabled={busy} onClick={() => void run(async () => { await revokeMarketKey(key.id); setConfirm('') })} className="text-danger">确认撤销</button>
