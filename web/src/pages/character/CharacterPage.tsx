        ...
        inputContent={
          surface === "bubble" ? (
            ...
            {(activePendingPermissions.length > 0 || activePendingQuestions.length > 0) && (
              <div className="fixed inset-0 z-[90] flex items-center justify-center bg-scrim/40 p-4 backdrop-blur-[2px]">
      ...
      <AnimatePresence>
          {historyOpen && (
            <motion.section
              ...
              className="fixed inset-0 z-40 flex flex-col bg-overlay/90 text-text shadow-none backdrop-blur-xl"
              style={noDragStyle}
            >
              <header className="flex items-center justify-between border-b border-highlight/10 px-4 py-4">
                <div className="flex min-w-0 items-center gap-2">
                  {historyView === "messages" ? (
                    <button
                      onClick={() => onSetHistoryView("sessions")}
                      className="rounded-lg p-2 text-text transition-colors hover:bg-highlight/10 hover:text-text"
                      aria-label="返回历史对话列表"
                      title="返回历史对话列表"
                    >
                  ...
                  <div className="min-w-0">
                    <div className="truncate font-serif text-lg text-text">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-5 py-3">
                {historyView === "sessions" ? (
                  <div className="mx-auto flex w-full max-w-[720px] flex-col gap-2 py-2">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          session.id === activeSessionId
                            ...
                            : "border-highlight/10 bg-highlight/5 text-text hover:border-highlight/20 hover:bg-highlight/10 hover:text-text",
                        )}
                      >
                    ...
                    {sessions.length === 0 && (
                      <div className="flex h-[50vh] items-center justify-center text-center text-sm text-text-muted">
                        还没有历史对话
                      </div>
                    )}
                  </div>
                      ...
                      <article
                        ...
                        className={cn(
                          ...
                          message.role === "user"
                            ...
                            : "mr-8 border-highlight/10 bg-highlight/7",
                        )}
                      >
                          ...
                          <div className="mb-3 flex flex-wrap gap-2">
                            {message.images.map((image, index) => (
                              <AttachmentImage
                                ...
                                className="max-h-28 max-w-full cursor-pointer rounded-lg border border-highlight/10 object-contain transition-opacity hover:opacity-85"
                              />
                            ))}
                          </div>
                        )}
                        {message.runtimeTrace && (
                          <details className="mb-3 rounded-xl border border-success/15 bg-success/10 px-3 py-2">
                            <summary className="cursor-pointer select-none text-[10px] uppercase tracking-[0.14em] text-success/80">
                              运行过程
                            </summary>
                            <div className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-text [overflow-wrap:anywhere]">
                              {message.runtimeTrace}
                            </div>
                          </details>
                        )}
                        {message.thinking && (
                          <details className="mb-3 rounded-xl border border-info/15 bg-info/10 px-3 py-2">
                            <summary className="cursor-pointer select-none text-[10px] uppercase tracking-[0.14em] text-info/80">
                              思考
                            </summary>
                            <div className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-text [overflow-wrap:anywhere]">
                              {message.thinking}
                        ...
                        {message.toolCalls && message.toolCalls.length > 0 && (
                          <div className="mb-3 grid gap-2">
                            {message.toolCalls.map((tool) => (
                              <details
                                key={tool.id}
                                className="rounded-xl border border-success/15 bg-success/10 px-3 py-2"
                              >
                                <summary className="cursor-pointer select-none text-[10px] uppercase tracking-[0.14em] text-success/80">
                                  工具：{tool.name}
                                ...
                                <div className="mt-2 grid gap-2">
                                  <div>
                                    ...
                                    <pre className="whitespace-pre-wrap rounded-lg border border-highlight/10 bg-scrim/20 p-2 text-xs text-text [overflow-wrap:anywhere]">
                                      {tool.input}
                                    </pre>
                                  </div>
                                  {tool.output && (
                                  ...
                                  {tool.error && (
                                    <div>
                                      <div className="mb-1 text-[10px] uppercase tracking-[0.12em] text-danger/80">
                                        错误
                                      </div>
                                      <pre className="whitespace-pre-wrap rounded-lg border border-danger/20 bg-danger/30 p-2 text-xs text-danger [overflow-wrap:anywhere]">
                                        {tool.error}
                                      </pre>
                                    </div>
                                  )}
                        ...
                        {message.content && (
                          <div className="whitespace-pre-wrap text-sm leading-relaxed text-text [overflow-wrap:anywhere]">
                            {message.content}
                          </div>
                        )}
                      </article>
