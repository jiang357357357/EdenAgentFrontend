...
export function ConnectorPermissions({ id, revisionHint, onChanged }: { id: string; revisionHint: string; onChanged?: () => void }) {
  ...
  return <section className="my-4 rounded-xl border border-border bg-card p-4 text-sm">
    ...
    {!snapshot && !error && <p className="mt-2 text-text-muted">读取权限中…</p>}
    {snapshot && <>
      <p className="mt-2 text-xs text-text-muted">授权仅适用于当前配置和包版本。修改配置后需重新授权；撤销权限会停用连接器。</p>
      ...
      <ul className="mt-2 space-y-3">{snapshot.permissions.map(permission => <li key={permission.key} className="border-t pt-3">
        ...
        <p className="break-all text-xs text-text-muted">资源：{permission.resolvedResource ?? '尚未配置'} · {permission.access}</p>
