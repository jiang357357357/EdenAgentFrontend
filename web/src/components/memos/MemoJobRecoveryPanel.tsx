...
export function MemoJobRecoveryPanel() {
  ...
  return <div className="relative text-sm">
    <button type="button" aria-expanded={open} onClick={() => setOpen(value => !value)} className="rounded-full border border-border bg-card px-3 py-2 text-text">提醒作业恢复</button>
    {open && <section aria-label="提醒作业恢复" className="absolute right-0 top-full z-50 mt-2 max-h-[65vh] w-[min(36rem,85vw)] overflow-auto rounded-lg border border-border bg-card p-3 text-text shadow-xl">
