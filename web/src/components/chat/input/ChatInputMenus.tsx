  ...
  return (
    <>
      {permissionMenuOpen && (
        <div
          ...
          className={cn(
            ...
            overlay ? "border-highlight/12 bg-overlay/88 text-text" : "border-border bg-card text-text",
          )}
        >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active
                    ? overlay ? "bg-warning/12 text-warning" : "bg-warning-dim text-warning"
                    : overlay ? "text-text hover:bg-highlight/8 hover:text-text" : "text-text-muted hover:bg-bg hover:text-text",
                )}
              >
      ...
      {modelMenuOpen && (
        ...
        >
          {modelError && <div className={cn("px-3 py-2 text-xs", overlay ? "text-danger" : "text-danger")}>{modelError}</div>}
          {modelLoading && !modelConfig ? (
            <div className={cn("px-3 py-3 text-sm", overlay ? "text-text" : "text-text-muted")}>正在读取模型...</div>
            ...
            <div className={cn("px-3 py-3 text-sm", overlay ? "text-text" : "text-text-muted")}>没有可用模型</div>
          ) : null}
        </div>
      )}
    </>
