      ...
      {!voicePanelVisible && (
        <div className={cn("absolute z-20 flex h-[5.4vh] items-center justify-between gap-[1.4vh]", overlay ? "inset-x-[2.4vh] bottom-[1.6vh]" : "bottom-[1.7vh] left-[2.4vh] right-[9.1vh]") }>
          <div className="flex min-w-0 items-center gap-[1.6vh]">
            <button type="button" onClick={onFilePick} disabled={isDialogMode || Boolean(disabled)} className={cn("flex h-[4.2vh] w-[4.2vh] flex-shrink-0 items-center justify-center rounded-[1.2vh] transition-colors disabled:cursor-not-allowed disabled:opacity-35", overlay ? "text-text/85 hover:bg-highlight/10 hover:text-text" : "text-text-muted hover:bg-bg hover:text-text")} aria-label="添加附件" title={disabled ? "当前回合中只能排队文字消息" : "添加附件"}>
              <Plus className="h-[2.8vh] w-[2.8vh]" />
            </button>
            <button type="button" onClick={onTogglePermissionMenu} className={cn("flex min-w-0 items-center gap-[0.8vh] rounded-[1.2vh] border border-transparent px-[1.15vh] py-[0.8vh] font-medium transition-colors", overlay ? "bg-scrim/15 text-warning hover:bg-warning/10" : "bg-transparent text-warning hover:bg-warning-dim")} aria-expanded={permissionMenuOpen} aria-haspopup="menu" aria-label={`当前权限: ${permissionLabel}`} title={`当前权限: ${permissionLabel}`}>
            ...
            {!hideOverlayActions && hasDialog && (
              <button type="button" onClick={onToggleOverlayMode} className="hidden h-[4.2vh] w-[4.2vh] flex-shrink-0 items-center justify-center rounded-[1.2vh] text-text/85 transition-colors hover:bg-highlight/10 hover:text-text sm:flex" aria-label={isDialogMode ? "切换到输入" : "切换到对话"} title={isDialogMode ? "切换到输入" : "切换到对话"}>
          ...
          <div className="flex flex-shrink-0 items-center gap-[1.5vh]">
            {outputActive || disabled ? <LoaderCircle className={cn("h-[2.5vh] w-[2.5vh] animate-spin", overlay ? "text-text/80" : "text-text-muted")} aria-label="正在处理" /> : null}
            {voiceInputEnabled && !overlay ? (
              <button type="button" onClick={onToggleVoiceInput} disabled={Boolean(disabled) || voiceStatus === "connecting" || voiceStatus === "transcribing"} className={cn("flex h-[4.2vh] w-[4.2vh] flex-shrink-0 items-center justify-center rounded-full transition-[color,background-color,transform] disabled:cursor-wait", voiceStatus === "recording" ? "bg-danger/10 text-danger" : voiceError ? "text-danger hover:bg-danger/10" : "text-text-muted hover:bg-bg hover:text-text")} style={{ transform: voiceStatus === "recording" ? `scale(${1 + voiceLevel * 0.12})` : undefined }} aria-label={voiceStatus === "recording" ? "停止录音" : "开始语音输入"} title={voiceError || (voiceStatus === "recording" ? "停止录音" : "语音输入")}>
            ...
            <button type="button" onClick={onToggleModelMenu} disabled={modelLoading && !modelConfigAvailable} className={cn("flex max-w-[18vh] items-center gap-[0.8vh] rounded-[1.2vh] px-[0.7vh] py-[0.8vh] text-[1.9vh] font-medium transition-colors disabled:cursor-wait disabled:opacity-60", overlay ? "text-text/85 hover:bg-highlight/10 hover:text-text" : "text-text-muted hover:bg-bg hover:text-text")} aria-expanded={modelMenuOpen} aria-haspopup="menu" aria-label={`模型: ${modelLabel}`} title={modelButtonTitle}>
              <span className="truncate">{modelLabel}</span>
              <ChevronDown className="h-[1.7vh] w-[1.7vh] flex-shrink-0" />
            </button>
            {overlay && !hideOverlayActions && (
              <>
                <button type="button" onClick={onHistory} className="hidden h-[4.2vh] w-[4.2vh] items-center justify-center rounded-[1.2vh] text-text/85 transition-colors hover:bg-highlight/10 hover:text-text sm:flex" aria-label="历史会话" title="历史会话"><History className="h-[2.1vh] w-[2.1vh]" /></button>
                {isDialogMode && <button type="button" onClick={onPreviousOutput} disabled={outputIndex === 0} className="hidden h-[4.2vh] w-[4.2vh] items-center justify-center rounded-[1.2vh] text-text/85 transition-colors hover:bg-highlight/10 hover:text-text disabled:opacity-30 sm:flex" aria-label="上一条" title="上一条"><ChevronLeft className="h-[2.1vh] w-[2.1vh]" /></button>}
                <button type="button" onPointerDown={onDragPointerDown} onPointerUp={onDragPointerCancel} onPointerLeave={onDragPointerCancel} onPointerCancel={onDragPointerCancel} onContextMenu={(event) => event.preventDefault()} className="hidden h-[4.2vh] w-[4.2vh] items-center justify-center rounded-[1.2vh] text-text/85 transition-colors hover:bg-highlight/10 hover:text-text sm:flex" aria-label="长按移动窗口" title="长按移动窗口"><Move className="h-[2.1vh] w-[2.1vh]" /></button>
              </>
            )}
