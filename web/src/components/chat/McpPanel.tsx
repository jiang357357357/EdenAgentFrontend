  ...
  return <section className="my-3 rounded-lg border p-3 text-sm">
    ...
    {open && <div className="mt-3 space-y-2">
      ...
      {runtimes.map(item => <div key={item.id} className="break-all border-t pt-2">
        ...
        <p className="text-xs text-text-muted">{item.server.name} {item.server.version} · 包版本 {item.revision.slice(0, 12)}</p>
      </div>)}
      {errors.map(item => <p key={item.id} className="break-all text-danger">{item.id}：{item.error}</p>)}
      {history && <>
        {!history.items.length && <p>当前会话暂无 MCP 调用记录。</p>}
        {history.items.some(item => item.state === 'unknown') && <p className="text-warning">结果未知的操作可能已经在远端执行，请先核对远端状态。</p>}
        <ul>{history.items.map(item => <li key={item.id} className="break-all border-t py-2">
          <p>{item.name} · {labels[item.state] ?? item.state}</p>
          <p className="text-xs text-text-muted">{item.runtimeId} · {new Date(item.updatedAt).toLocaleString()}</p>
