...
test('persisted tool events expose only actual executions and results', () => {
  const calls = selfAwakeToolExecutions({events:[
    {kind:'agent.tool_execution_start',payload:{toolCallId:'call-1',toolName:'get_self_awake_context'}},
    {kind:'agent.tool_execution_end',payload:{toolCallId:'call-1',result:{status:'ok',items:2},isError:false}},
    {kind:'turn.completed',payload:{}},
  ]})
  assert.deepEqual(calls.map(({args, rawResult, ...call}) => call), [{id:'call-1',name:'get_self_awake_context',status:'succeeded',result:'{"status":"ok","items":2}'}])
})
