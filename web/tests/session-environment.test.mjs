...
const api = readFileSync(new URL("../../../Server/crates/mon-agent-api/src/lib.rs", import.meta.url), "utf8")
const store = readFileSync(new URL("../../../Server/crates/mon-agent-store/src/lib.rs", import.meta.url), "utf8")
const turnPreparation = readFileSync(new URL("../../../Server/crates/mon-agent-app/src/runtime/turn/prepare.rs", import.meta.url), "utf8")
...
test("session environment is durable and enters the model context without raw coordinates", () => {
  ...
  assert.doesNotMatch(
    readFileSync(new URL("../../../Server/crates/mon-agent-app/src/prompt.rs", import.meta.url), "utf8"),
    /用户地点[^\n]*latitude|用户地点[^\n]*longitude/,
  )
})
