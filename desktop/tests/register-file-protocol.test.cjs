...
test("registerFileProtocol maps edenagent-file requests to net.fetch", async () => {
  let scheme = null
  let handler = null
  const protocol = {
  ...
  const result = handler({ url: "edenagent-file:///D:/Mon/example.png" })

  assert.equal(scheme, "edenagent-file")
